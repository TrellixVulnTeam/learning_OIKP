    ...
    def callback(self,session,user_id,payment,category,item):
        ...
        if user.find_one({"_id":user_id})['savings'] >=payment:
            if inventory.find_one({f"{category}.{item}.stock":{"$gte":1}}) :
                inventory.update_one({f"{category}.{item}.stock": {"$gte":1}}, {"$inc": {f"{category}.$.{item}.stock": -1}})
...
#I will run this file first to create docs.
...
#         client.trans.users.insert_one(a)

#     create_ = MongoTrans()
#     create_.client.trans.inventory.insert_one({"grocery":[{"coffee":{"stock":1000,"price":2200}},
...
#     for i in range(100):
#         b = fake.profile()
#         b.pop('current_location')
#         b.pop('birthdate')
#         b.update({"ordered":[],"savings" : i*random.randint(500,50000)})
#         a.client.trans.users.insert_one(b)
